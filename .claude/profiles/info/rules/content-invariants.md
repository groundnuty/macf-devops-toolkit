...
- **INV-1: Palette sync.** Color names in `preamble.tex` must match SCSS variables in `**/*.md`. Verify with `./scripts/check-palette-sync.sh`. Any new color added to one must be added to the other.
