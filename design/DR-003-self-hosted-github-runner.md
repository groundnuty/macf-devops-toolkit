...
### The grant is per-agent (multi-path), derived from the registry

The router (macf-actions#53) invokes `${workspace_dir}/.claude/scripts/tmux-send-to-claude.sh` **as the target agent's `ssh_user`, per target agent** — and these are per-workspace copies at distinct abspaths (`macf-devops-toolkit/…`, `macf-science-agent/…`, `macf/…`). `sudo` matches the **literal** command path, so a single-path grant would silently deny inject to every *other* agent. `phase_setup` therefore enumerates the helpers from the **same registry the router reads** (`.github/agent-config.json`), grants one sudoers line per `ssh_user` (`macf-runner ALL=(<ssh_user>) NOPASSWD: <helper1>, <helper2>, …`), and asserts each helper unreplaceable. Local (existing) helpers only — an off-VM agent uses SSH, not this path. All current on-VM agents (devops / science / code) run as `ubuntu`, so it is one line with three abspaths. Re-run `make github-runner-setup` when the agent set changes. The byte-identical-abspath requirement cuts both ways: #53 must invoke the literal registry `workspace_dir` path (no realpath/symlink normalization) to match the grant.
...
**Requirement (the bound's precondition on the routing side):** on `runner.environment == 'self-hosted'`, routing must inject by calling the helper **locally** — `sudo -u $AGENT_OWNER "$HELPER" "$TARGET" "$PROMPT"` (base64-decoded per #47), the exact invocation #91's single-command sudoers grant authorizes — and must **not** materialize `AGENT_SSH_KEY` on the box. "Not materialize" includes the **job env**, not just the key file: the job-level `env: AGENT_SSH_KEY: ${{ secrets.AGENT_SSH_KEY }}` exposes the secret via `/proc/self/environ` regardless of which inject branch runs, so a self-hosted RCE reads it even if `~/.ssh/id_agent` is never written — the secret-ref must move to the hosted-only step (step-level env) or be conditional-emptied on self-hosted. This is the no-SSH direct-helper follow-up #52 offered; it is a **hard gate**, not an optimization.

**#91 needs no change** — its sudoers grant (`macf-runner ALL=($AGENT_OWNER) NOPASSWD: $HELPER`) is purpose-built for exactly this local invocation. The reconciliation is on the routing side (code-agent): land the self-hosted no-SSH branch and ensure `AGENT_SSH_KEY` is not exposed to the self-hosted arm (file **and** job env) — **tracked as macf-actions#53**.
