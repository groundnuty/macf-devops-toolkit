...
- **Related:** `macf-devops-toolkit#105` (phase-0 task), `#90` (self-hosted runner — separate workstream), `macf#528` (repo-scope erratum, science stewarding the DR-005/006 canonical amendment)
...
**Rotation (asymmetric, repo-scope reality per `macf#528`).** Leaf certs rotate **on the VM** under the long-lived CA — no GitHub touch (cheap; can be automated/tighter than DR-004 default). The routing **client** cert is a GHA secret: rotating it re-pushes to each caller repo (`groundnuty` is a user account, not an org, so there is no single org-secret rotation point — a 4-repo `for gh secret set` loop). The CA rotates only on compromise (5-yr validity; rotating it forces all leaves + the `MACF_<PROJ>_CA_CERT` Variable to re-issue).
...
## Decision 4 — Registry + secret/Variable layout (repo-scoped, per `macf#528`)

`groundnuty` is a **user account, not an org** (`/orgs/groundnuty` → 404), so registry + secrets are **repo-scoped**, overriding the v3 `/orgs` default and DR-027's original org-scoped wording (science stewards the `macf` DR-005/006 canonical amendment).

- **`registry-api-path = /repos/groundnuty/macf`** per caller (the auditor's `env.registry` is already `MACF_REGISTRY_TYPE=repo`, `MACF_REGISTRY_REPO=groundnuty/macf`).
- **Variables** (read by the router on `groundnuty/macf`):
  ...
  - `MACF_AGENT_<NAME>` — **self-registered by each channel server on launch** (value = `{host,port,type,instance_id,started}` JSON). **Devops does not hand-set these.**
- **Secrets** (per-repo, on each caller repo):
...
- **Caller workflow:** `agent-router.yml` pinned **`@v3.3.0`** with `with: { project: macf, registry-api-path: /repos/groundnuty/macf }`.

**Open registry-scope point for science's canonical amendment:** the auditor registers on `groundnuty/macf` (the framework repo), *not* its own `macf-auditor-agent` repo. Whether each agent should register on `macf` (one shared registry repo) vs. its own repo (profile/per-repo scope) is a DR-005/006 concept call — flagged, not decided here.
...
1. **Mint the project CA** (if absent): `macf certs init` for project `macf` → writes `~/.macf/certs/macf/ca-{cert,key}.pem` (`0600`) and uploads the `MACF_CA_CERT` (+ `MACF_CA_KEY_ENCRYPTED`) Variable to `groundnuty/macf`. *(Confirmed gap: `~/.macf/certs/macf/` does not exist yet — init skipped leaf-gen because no CA was present.)*
...
4. **Set per-agent env** in `.claude/.macf/`: `MACF_PORT=8700`, `MACF_ADVERTISE_HOST=orzech-dev-agents.tail491af.ts.net`, `MACF_HOST=0.0.0.0`; cert + registry pointers already wired.
5. **Caller workflow + repo config:** `agent-router.yml@v3.3.0` on the auditor's caller with `project: macf`, `registry-api-path: /repos/groundnuty/macf`; ensure the repo carries `MACF_ROUTING_APP_*` + `TS_OAUTH_*`.
6. **Issue + install `ROUTING_CLIENT_*`** as secrets on the caller repo(s).
7. **Launch** `claude.sh` (tmux `macf@auditor`) → the channel server binds `:8700` and self-registers `MACF_AGENT_AUDITOR` on `groundnuty/macf`. **[OPERATOR GATE: the `MACF_ROUTING` App must exist + my devops App must have access to the relevant repos — see below.]**
...
1. **`MACF_ROUTING` App** (read-only: `metadata:read` + repo-`actions_variables:read`) — its `*_APP_ID/_KEY` secrets are **already set per-repo** (verified). Confirm the App itself exists + is installed where the router runs.
2. **App install scope:** for devops to manage the auditor repo's Variables/secrets and push its caller workflow, the `macf-devops-agent` App likely needs **access to `macf-auditor-agent`** (it is not in my installation's repo selection — a scoped-App 404, not absence). Some auditor config is filesystem-side (certs, env) where no repo access is needed; the precise list is: caller-workflow push + secret/variable set → needs repo access.
3. **CA/leaf minting** runs on `orzech-dev-agents` (devops-executed; no operator action beyond the above).

## Open questions

- **Registry scope** (`macf` DR-005/006 amendment, science): each agent registers on `groundnuty/macf` vs. own repo; repo vs. profile scope for a user account.
...
- **Org-scoped registry/secrets (DR-027 original wording)** — N/A: `groundnuty` is a user account → repo-scoped (`macf#528`).
...
- `macf-devops-toolkit#105` (phase-0), `#90` (runner), `macf#525` (DR-027 ratification), `macf#526` (epic), `macf#528` (repo-scope erratum)
